...
public class CourseApplicationServiceImpl implements CourseApplicationService {
    ...
    public Result<Object> getApplicationList(Integer page, Integer size, String keyword) {
        ...
        try {
            ...
            // 只查询当前教师的申请
            List<CourseApplication> applications = courseApplicationMapper.selectPageByTeacherId(offset, size, keyword, currentUserId);
            
            // 统计当前教师的申请总数
            CourseApplication queryApplication = new CourseApplication();
            queryApplication.setTeacherId(currentUserId);
            long total = courseApplicationMapper.selectCount(queryApplication);
            
            System.out.println("getApplicationList: 查询结果总数 = " + total);
            System.out.println("getApplicationList: 当前页记录数 = " + applications.size());
            
            PageResult<CourseApplication> result = PageUtil.createPageResult(page, size, total, applications);
    ...
    public Result<Object> getApplicationListForAdmin(Integer page, Integer size, String keyword) {
        int offset = PageUtil.calculateOffset(page, size);
        List<CourseApplication> applications = courseApplicationMapper.selectPage(offset, size, keyword);
